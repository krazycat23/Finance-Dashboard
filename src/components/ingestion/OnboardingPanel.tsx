...
export function OnboardingPanel(){
  ...
  return <Panel><PanelHeader title="Company onboarding" meta="Local staging · review · validation · activation"/><PanelBody>
    <div className="mb-4 flex flex-wrap items-end gap-3"><label className="text-[11px] text-secondary">Company<input className="mt-1 block h-8 w-48 border border-line bg-panel px-2 text-primary" value={company.profile.companyName} onChange={e=>setCompany({...company,profile:{...company.profile,companyName:e.target.value,shortName:e.target.value}})}/></label><label className="inline-flex h-8 items-center gap-2 border border-line px-3 text-[12px] text-secondary cursor-pointer hover:border-strong"><Upload size={14}/> Upload CSV / XLSX<input className="hidden" type="file" accept=".csv,.xlsx,.xls" multiple onChange={e=>void upload(e.target.files)}/></label><button className="h-8 border border-line px-3 text-[12px] hover:border-strong" onClick={()=>{const fresh=makeCompany();setCompany(fresh);setWorkspace(emptyWorkspace(fresh));}}>New company</button></div>
    {saved.length>0&&<div className="mb-3 text-[12px] text-secondary">Reopen workspace: {saved.map(item=><button key={item.company.id} className="ml-2 border border-line px-2 py-1 hover:border-strong" onClick={()=>{void store.load(item.company.id).then(latest=>{if(latest){setCompany(latest.company);setWorkspace(latest);}}).catch(cause=>setError(String(cause)));}}>{item.company.profile.companyName}{item.activatedDataset?" · Activated":" · Draft"}</button>)}</div>}
    <div className="mb-3 grid gap-2 text-[12px] md:grid-cols-3"><div className="border border-subtle p-2">{workspace.sources.length} files · {workspace.datasets.length} datasets</div><div className="border border-subtle p-2">{workspace.mappings.filter(m=>m.status==="mapped").length} confirmed field mappings</div><div className="border border-subtle p-2"><Badge tone={blockers.length?"negative":"positive"}>{blockers.length?`${blockers.length} blocking`:"Ready"}</Badge> · {result.reconciliations.length} checks</div></div>
    ...
    {workspace.datasets.some(d=>d.wideUnpivot)&&<div className="mb-3 border border-subtle p-3 text-[12px]">Wide-table identifier/value selection is retained in workspace configuration.</div>}
    ...
    {blockers.length>0&&<div className="mt-3 border border-negative p-3 text-[12px] text-negative">{blockers.map(issue=><div key={issue.id}>{issue.message}</div>)}</div>}
    {error&&<p role="alert" className="mt-3 text-xs text-negative">{error}</p>}
    <div className="mt-4 flex gap-2"><button className="h-8 bg-accent px-3 text-[12px] text-accent-on disabled:opacity-40" disabled={busy||activationBlockers(result).length>0} onClick={()=>{setBusy(true);void activateWorkspace({...workspace,company}).then(()=>navigate("/")).catch(cause=>setError(String(cause))).finally(()=>setBusy(false));}}><CheckCircle2 className="mr-1 inline" size={14}/>Activate reporting dataset</button><button className="h-8 border border-line px-3 text-[12px] hover:border-strong" onClick={()=>void activateDefaultDataset().catch(cause=>setError(String(cause)))}>Switch to demo</button></div>
  </PanelBody></Panel>;
}
