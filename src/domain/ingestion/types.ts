...
export type DatasetType = "finance_actual" | "budget" | "forecast" | "sales" | "operational_kpi" | "account_master" | "entity_master" | "product_master" | "customer_master" | "location_master" | "financial_calendar" | "unknown";
...
export interface StagedDataset { id: string; sourceFileId: string; sourceSheet?: string; columns: ColumnProfile[]; rows: ReadonlyArray<Record<string, unknown>>; inferred: Classification; confirmedType?: DatasetType; status: ImportStatus; warnings: string[]; errors: string[]; tableRange?: TableRange; }
export interface WideUnpivotConfiguration { identifierColumns: string[]; valueColumns: string[]; periodFromColumn: boolean; scenarioFromColumn?: boolean; valueField?: string; }
...
export interface MappingRule { id: string; companyId: string; priority: number; kind: "exact" | "prefix" | "range" | "nameContains"; value: string; statement: "pnl" | "balance" | "cashflow"; line: StatementLine; sign: 1 | -1; /** Converts the source’s natural-sign value to the canonical stored magnitude. */ sourceMultiplier?: 1 | -1; }
...
export interface IngestionReconciliation { id: string; label: string; sourceTotal: number; targetTotal: number; difference: number; differencePercent?: number; tolerance: number; status: "Reconciled" | "Within tolerance" | "Exception"; }
