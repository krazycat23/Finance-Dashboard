...
import { buildFinanceReconciliations } from "./transform/reconciliation";
...
export function buildImportedDataset(workspace:ImportWorkspace):{dataset?:ReportingDataset;issues:IngestionIssue[];reconciliations:IngestionReconciliation[]}{
  ...
  for(const check of reconciliations.filter(check=>check.status==="fail"))issues.push(issue(check.datasetId??"finance",`reconciliation:${check.id}`,`${check.label} failed by ${check.difference.toFixed(2)} (tolerance ${check.tolerance}).`,check.sourceRowCount??0));
  ...
  const reconciliationScore=reconciliations.length&&reconciliations.every(check=>check.status==="pass")?1:0;
  // Transparent data-health score: all source value must be mapped and every
